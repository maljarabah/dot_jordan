...
function AddProjectModal({ state, setState, isRefetch, setIsRefetch }) {
    ...
    const handleCreate = async () => {
        setError(null)
        try {
            await axios.post(import.meta.env.VITE_SERVER_URI + "/api/projects", formData, { headers: { Authorization: `Bearer ${user.token}` } })
            ...
            <div className="fixed left-[50%] top-[50%] mx-auto w-full max-w-lg translate-x-[-50%] translate-y-[-50%] px-4">
                <div className="rounded-md bg-white shadow-lg">
                    <div className="flex items-center justify-between border-b p-4">
                        <p className="text-lg font-medium text-gray-800">Create a new project</p>
