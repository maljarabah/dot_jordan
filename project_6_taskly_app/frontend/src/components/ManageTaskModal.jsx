...
function ManageTaskModal({ state, setState, isRefetch, setIsRefetch, taskData }) {
    ...
    const handleUpdate = async () => {
        setError(null)
        try {
            await axios.patch(import.meta.env.VITE_SERVER_URI + `/api/tasks/${taskData._id}`, formData, { headers: { Authorization: `Bearer ${user.token}` } })
            setIsRefetch(!isRefetch)
            setError(null)
            setFormData({ name: "", description: "", dueDate: "", status: "", priority: "", projectId: "" })
            setState(false)
        } catch (err) {
    ...
    const handleDelete = async () => {
        ...
        if (confirm("Are you sure you want to delete this task?")) {
            const response = await axios.delete(import.meta.env.VITE_SERVER_URI + `/api/tasks/${taskData._id}`, { headers: { Authorization: `Bearer ${user.token}` } })
            ...
            <div className="fixed left-[50%] top-[50%] mx-auto w-full max-w-lg translate-x-[-50%] translate-y-[-50%] px-4">
                <div className="rounded-md bg-white shadow-lg">
                    <div className="flex items-center justify-between border-b p-4">
                        <p className="text-lg font-medium text-gray-800">Task Settings</p>
                    ...
                    <div className="my-4 flex flex-col gap-3 px-4 text-[15.5px] leading-relaxed text-gray-500">
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <input type="date" name="dueDate" id="dueDate" value={formatDateForInput(formData.dueDate)} onChange={handleChange} className="w-full rounded-lg border bg-transparent px-3 py-2 text-gray-500 outline-none focus:border-blue-600" required />
                        </div>
                        <div className="flex items-center gap-4">
                        ...
                        <div className="flex items-center gap-4">
                            <label htmlFor="priority" className="min-w-32 font-semibold text-gray-600">
                                Priority
                            </label>
                            <select name="priority" id="priority" className="w-full rounded-lg border bg-transparent px-3 py-2 text-gray-500 outline-none focus:border-blue-600" value={formData.priority} onChange={handleChange}>
